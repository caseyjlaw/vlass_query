...
def get_filename(co, tilename=None):
    ...
    res = requests.get('{0}/quicklook/VLASS1.1/{1}/'.format(archive_url, tilename))
    ...
    for line in lines:
        ...
        co2 = coord.SkyCoord('{0}:{1}:{2}'.format(center[1:3], center[3:5], center[5:7]),
                             '{0}:{1}:{2}'.format(center[7:10], center[10:12], center[12:14]),
                             unit=(u.hour, u.deg))
...
def parse_tab():
    ofektab = pd.read_table('ofek_table1.txt', delim_whitespace=True, na_filter=False, comment='#', index_col=False,
                            names=['ra0', 'ra1', 'ra2', 'dec0', 'dec1', 'dec2', 'Sf', 'eSf', 'L','z', 'offset']) 
    for i in range(len(ofektab)):
        row = ofektab.iloc[i]
        co = coord.SkyCoord((row.ra0, row.ra1, row.ra2), (row.dec0, row.dec1, row.dec2), unit=(u.hour, u.deg))
        try:
    ...
    with open('ds9.reg', 'a') as fp:
        for i in range(len(ofektab)):
            ...
            fp.write("circle({0}:{1}:{2},{3}:{4}:{5},30\")\n"
                     .format(int(row.ra0), int(row.ra1), row.ra2, int(row.dec0), int(row.dec1), row.dec2))
